...
const AdminCommentChannelCard = ({ onClose, postId }) => {
    ...
    const onClickAdminComment = async ({ postId }) => {
        ...
        try {
            const response = await axios.post(`${REACT_APP_API_URL}/channelRequestBoard/addAdminComment`, body, { withCredentials: true });
    ...
    const handleState = async ({ postId }) => {
        ...
        try {
            const response = await axios.post(`${REACT_APP_API_URL}/channelRequestBoard/updateStateByAdmin`, body, { withCredentials: true });
            switch (response.data) {
                case 0:
                    alert("처리 성공");
                    onClose();
