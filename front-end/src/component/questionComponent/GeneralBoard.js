...
const onClickDeleteGeneralNotice = async ({ userId, postId }) => {
    ...
    try {
        const response = await axios.post(`${REACT_APP_API_URL}/board/deletePost`, body, { withCredentials: true });
...
const GeneralBoard = () => {
    ...
    const fetchData = async () => {
        const userId = sessionStorage.getItem("userId");
        try {
            const response = await axios.get(`${REACT_APP_API_URL}/board/searchPost`, {
                params: {
                    userId: userId,
                },
